...
FixStyle(spectrum/atom,FixSpectrumAtom)

#else

#ifndef LMP_FIX_SPECTRUM_ATOM_H
#define LMP_FIX_SPECTRUM_ATOM_H
...
class FixSpectrumAtom : public Fix {
 public:
  FixSpectrumAtom(class LAMMPS *, int, char **);
  ~FixSpectrumAtom();
  int setmask();
  void init();
  void setup(int);
 ...
 private:
  int neval,nvalues, me, overwrite;
  double omega_lo, omega_hi;
  int omega_n,omega_scale;
  bigint nvalid,nvalid_last;
  void copy_arrays(int, int, int);
  int *which,*argindex,*value2index;
  char **ids;
  double **array;
  double ***v_omega_r,***v_omega_i;
  double *omega_vec;
  int count;
  FILE *fp;
  long filepos;
